...
\paragraph{Notations}
We denote by $M_{ij}$ \textbf{a} \textit{governing field} of the map $p \to a_{ij}(p)$.
...
Here, we use the various computations results that are discussed in the next section \ref{numerics:GoverningFields}.
...
However, we will write equality sign, to lighten notation.

\paragraph{Brute Results}
We will provide details in here only for $a_{03}$ and $a_{30}$.
...
\begin{conjecture}[Diagonal Governing Groups Conjecture]
	For all $k \in \N^*$, there exists a fields $M_{0k}$ such that $M_{0k}$ is a governing field for $a_{0k}$, and $G_{0k} = \Gal{M_{0k}/\Q}$.
	For all $k \in \N^*$, there exists a fields $M_{k0}$ such that $M_{k0}$ is a governing field for $a_{k0}$, and $G_{k0} = \Gal{M_{k0}/\Q}$.
