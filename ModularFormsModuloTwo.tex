...
\begin{theorem}[Order of Nil-potency of Modular Forms Modulo 2]
	Let $f \in \mathcal{F}$ be a non-zero modular form modulo 2.
...
As $g(f)=h(f)+1$, and $h(f) = h(\degree{f}) = \mathcal{O}(\sqrt{\degree{f}})$, we have $g(f) = \mathcal{O}(\sqrt{\degree{f}})$, i.e. the nil-potency order of a modular form behaves asymptotically as the square root of its degree.
