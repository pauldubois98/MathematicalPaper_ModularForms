...
Non-nullity of Hecke operators is also implied by the following property, which is new to this paper:
\begin{property}
...
\subparagraph{Variation of $h$}
By construction (it is simple to see with the example above), $h(n+1) \geq h(n)$.
Note that it is $\underline{\text{not true}}$, in general, that $n_3(n+1) \geq n_3(n)$ nor $n_5(n+1) \geq n_5(n)$.
...
Then $m \leq 2^{k_1+1}$, so $h(m) \leq h(2^{k_1+1}) \leq \sqrt{2^{k_1}} \leq \sqrt{m}$.
So in general, we have:
$$
h(m) \leq \sqrt{m} \quad \text{for all } m \in \N.
$$

\subparagraph{Asymptotic Behaviour of $h$}
Thus, we have:
$$
\nicefrac{1}{2}\sqrt{m} \leq h(m) \leq \sqrt{m}
\qquad \text{for all } m \in \N.
$$
...
In this case, $m_1$ is the \textit{degree} of $f$, and we will denote it by $\partial f$.
We write $\partial f = -\infty$ if $f=0$.
...
Here, we want to give a description for the behaviour of $h(T_3|f)$ and $\partial T_3|f$.
\begin{proposition}
	Let $f \in \mathcal{F}$ be a non-zero modular form modulo 2.
	\begin{enumerate}
		\item In general, $h(T_3|f) \leq h(f)-1$.
		\item If $n_3(f)>0$, then $h(T_3|f) = h(f)-1$ and $\partial T_3|f$ has code $\left[ n_3(f)-1, n_5(f) \right] $.
	\end{enumerate}
\end{proposition}
...
Similarly to the above for $T_3$, we want to give a description for the behaviour of $h(T_5|f)$ and $\partial T_5|f$.
\begin{proposition}
	Let $f \in \mathcal{F}$ be a non-zero modular form modulo 2.
	\begin{enumerate}
		\item In general, $h(T_5|f) \leq h(f)-1$.
		\item If $n_5(f)>0$, then $h(T_5|f) = h(f)-1$ and $\partial T_5|f$ has code $\left[ n_3(f), n_5(f)-1 \right] $.
	\end{enumerate}
\end{proposition}
...
\begin{proof}
	...
	Therefore, $\partial g > -\infty$, so it must be odd. since it has code $\left[ 0,0 \right]$,  $\partial g = 1$, thus $g = \Delta$.
\end{proof}
...
\begin{proof}
	...
	Thus, $\partial f$ has code $\left[ 0,0 \right]$.
