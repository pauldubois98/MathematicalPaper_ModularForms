...
It is not trivial, at this point, why and how we can reduce modulo two modular forms, objects that have coefficients in $\C$.
In general, reduction modulo a number is only possible with whole numbers (integers).
We would like to reduce modulo two coefficients of the Fourier series for modular forms.
...
Once this is done, we will reduce all the newly integers coefficients modulo $2$.

In this section, we will denote all object reduced with an $\overline{\text{over-line}}$: $f \to \overline{f}$; $c \to \overline{c}$; $T_n \to \overline{T_n}$.



\subsection{Integer Basis}
...
We would like to normalize this series, so that the coefficients become integers, so that we can ultimately reduce them modulo two.
...
If we multiply by $2$ all coefficients of a modular form that already lie in $\Z$, the reduction $\mod 2$ will always give zero.
...
Both $F_k$ and $E_k$ are interesting, but for our purpose (reducing modulo two), we will use $E_k$.
...
\paragraph{Normalisation of $\varDelta$}
Again, we recall the formula for $q$ extension of $\varDelta$:
$$
\varDelta(q) = (2\pi)^{12} q \prod_{n=1}^{\infty} (1-q^n)^{24}
$$
Again, we want to normalize the series so that the first coefficient is one. 
As there is no constant coefficients, we will normalize it so that the $q$ coefficient becomes $1$.
Therefore, we define $\varDelta = (2\pi)^{12}.\Delta$, so explicitly:
$$
...
\paragraph{Basis with Integer Coefficients (in Fourier Series)}
...
Therefore, we have found a basis for $M_k$ such that all elements in the basis have only integer coefficients in their $q$ expansion.
\label{IntegerBasisModularForms}
...
It was shown before that $\{E_2^aE_3^b | 2a+3b=k\}$ is a basis for $M_k$ with integer coefficients (see \ref{IntegerBasisModularForms})
...
$k \cong 4 \mod 12$ so $a=2$ and $b=0$; $d=2$.
...
\begin{align*}
    g_1(q) &= \Delta(q)E_2^2(q)E_3^2(q)\\
           ...
           & \qquad \qquad * \left[ 1 - 504q - 16632q^2 + 122976q^3 + O(q^4) \right]^2\\
...
% (Wolfram|Alpha) develop q - 552q^2 - 188244q^3 + 552*[q^2 + 432q^3]
\end{align*}
...
\begin{align*}
    f_0(q) &= g_0(q) - 1920g_1(q) - 1630080g_2(q)\\
           ...
           &= 1 + 2611200q^3 + O(q^4)
% (Wolfram|Alpha) develop [1 + 1920q + 1630080q^2 + 803228160q^3] - 1920*[q + 50220q^3] - 1630080*[q^2 + 432q^3]
\end{align*}
...
Now that we have a basis with integer coefficients, it makes sense to reduce forms modulo two.
For a modular form $f$, we denote its reduced modulo two from $\overline{f}$. It is defined as follows:
...
\qquad \text{ with } \ \overline{c}(n) = c(n) \mod 2 .
$$

We want to reduce Miller basis modulo two.
...
$$
\overline{E_2}(q)
= \overline{ 1 + 240 \sum_{n=1}^{\infty} \sigma_{3}(n)q^n } 
= 1 \mod 2
$$
...
$$
\overline{E_2}(q)
= \overline{ 1 - 504 \sum_{n=1}^{\infty} \sigma_{5}(n)q^n }
= 1 \mod 2
$$
...
\[
\Delta(q) \equiv \overline{\Delta}(q) = \sum_{m=0}^{\infty} q^{(2m+1)^2} \mod 2
\label{eq:Delta}
\]
...
But $\overline{E_2^a} = \overline{E_2}^a = 1^a = 1 \mod 2$ and similarly, $\overline{E_3^{2(d-j)+b}} = 1 \mod 2$.
So once the above set is reduced modulo two, we are left with $\{ \overline{\Delta}^j | 1 \leq j \leq \dim(M_k) \}$.
...
Now, if we look at this set not reduced modulo two, we have:
...
We understand now that modulo two, the weight of modular form doesn't make sense any more.
This is one of the consequences of reducing modulo two: we lose some informations about the modular forms, such as the weight.

From this observation, we should study all modular forms together, modulo two (instead of separating by weights).
This is why the space of modular forms modulo two will be denoted  $\mathcal{F}$, with mo dependence on $k$.

\subsubsection{Powers of $\Delta$}
\paragraph{Set of Powers of $\Delta$}
As we just saw, the Gaussian elimination of powers $\overline{\Delta}^k$ up to $\dim(\overline{M_k})$ form the Miller basis of $\overline{M_k}$ (modular forms of weight $2k$ reduced modulo two).

To lighten the notation, we will now write $\Delta$ instead of $\overline{\Delta}$, and consider everything modulo two.
For simplicity again, we will just take the powers of $\Delta$ to be our basis for modular forms modulo two (i.e. drop the Gaussian elimination process).
...
= \left( \sum_{m=0}^{\infty} q^{(2m+1)^2} \right)^k \mod 2
$$
Thus, we have $\tau(n)=\tau_1(n)$.

\paragraph{Proportion of zeros}
In fact, most of the coefficients $\tau_k(n)$ are $0$ modulo two.
...
\begin{table}[!ht]
	...
	\begin{center}
		\begin{tabular}{|r||c|c|c|c|c|c|c|c||l|}
			...
			\hline
			$a^2=$ & \color{BrickRed} $0$ & \color{ForestGreen} $1$ & \color{BrickRed} $4$ & \color{ForestGreen} $1$ & \color{BrickRed} $0$ & \color{ForestGreen} $1$ & \color{BrickRed} $4$ & \color{ForestGreen} $1$ & $\mod 8$ \\
			\hline
...
\end{table}
We remark that odd squares are all $1 \mod 8$, and even squares are all $0 \mod 8$.
...
This means, that $\Delta^k$ may only have terms $q^n$ such that $n \equiv k \mod 8$, i.e. $\Delta^k$ may only have terms of power congruent to $k \mod 8$.
When $k=1$, this is that $\Delta$ may only have terms of power $1 \mod 8$, this matches with table \ref{table:SquaresMod8}: all odd squares are $1 \mod 8$.
...
Therefore, we can write any modular form modulo two $f$ as the following:
$$
...
We define the space of modular forms modulo two denoted $\mathcal{F} $ to be\cite[2.1]{OrdreNilpotenceOperateurHecke}:
$$
\mathcal{F}
...
\subsubsection{Duality $\Delta$ - $q$}
%reference to wave-particule duality from physics
As we defined $\mathcal{F}$ above, a modular form modulo two is an expression of powers $\Delta^k$.
But we had from before that $\Delta = \sum_{m=0}^{\infty} q^{(2m+1)^2} \mod 2$.
Therefore, we can translate a modular form given as a finite polynomial of $\Delta$ into an infinite polynomial of $q$.
Thus, there are two ways to write a modular form modulo two.

This duality between the two definitions is what makes the study of modular forms modulo two so interesting:
...
Now that we have reduced modular forms modulo two, we would like to study the Hecke operators on these reduced modular forms. We define Hecke operators modulo two as follows:
...
We want to check that all the definitions make sense. When we look at $T_p|f$, there is a number of ways to to reduce it modulo two: $\overline{T_p|f}$, $\overline{T_p|\overline{f}}$, $\overline{\overline{T_p}|f}$, $\overline{T_p}|\overline{f}$.
...
We see now that looking at Hecke operators modulo two only for primes simplifies the sum to a computable formula.

As both $1$ and $p$ are odd, the term $a^{2k-1}$ reduces to $1$ modulo two.
We understand why Hecke operators modulo two isn't defined for even numbers: many terms in the summation would become zero.
...
All reductions give in fact the same result, so it makes sense to reduce modular forms modulo two, and still study the Hecke operators (but now only for odd primes).
As this all makes sense, we will now write only consider modular forms modulo two, and we will drop the over lines for simplicity.

\subsubsection{Properties}
What makes Hecke operators interesting, is that they have many properties.

\paragraph{Inherited properties}
From the fact that $\overline{T_p}|f(q) = \overline{T_p|f(q)}$, we get that the Hecke operators modulo two keep all the properties they had before being reduced.
...
This is because from definition, $T(n)f$ is a sum of modular forms (which remain modular).
Therefore, Hecke operators modulo two will as well transform a modular form to an other.
This was not clear from the definition modulo two that we had (which was in terms of $q$ series).
...
$$
Therefore, the Hecke operators modulo two commute.
...
Take a modular form modulo two $\overline{f}$ with degree $k$ (in terms of $\Delta$).
...
$$
From the definition of Hecke operator modulo two (\ref{DefHeckeOperatorsMod2}), we have:
$$
T_p|f
...
\begin{enumerate}
	...
	\item [$c(n/p)$:] As $p$ is an odd prime, it is an odd number, so from \ref{ObservationsMod8}, $p^2 \equiv 1 \mod 8$, so $p^{-2} \equiv 1 \mod 8$ as well (with $p^{-2}$ seen $\bmod 8$).
	
	Therefore, $np \not\equiv i \mod 8 \implies  n/p \equiv \nicefrac{np}{p^2} \equiv np \not\equiv i \mod 8 $.
	
	\item [$\gamma(n)$:] We conclude that $n \equiv np^2 \not\equiv pi \mod 8 \implies \gamma(n) = 0$
\end{enumerate}
Using \ref{ObservationsMod8} again, we deduce that $T_p|f \in \mathcal{F}_j$ with $j \equiv pi \mod 8$.

Overall, we have the following:
$$
f \in \mathcal{F}_i \implies T_p|f \in \mathcal{F}_j
\text{ with } j \equiv pj \mod 8
$$
...
\]
(since $\Delta^k \in \mathcal{F}_i \text{ with } k \equiv i \mod 8$)
...
$$
So $c(p) = \#\{m_1, m_2, m_3 \text{ odds } | m_1^2 + m_2^2 + m_3^2 = p\} \mod 2$ corresponds ($\bmod 2$) to the number of ways to write $p$ as sum of three odd squares.
...
\begin{align*}
	p \equiv 7 \bmod 8: & \quad T_p|\Delta^5 = \Delta^3 \text{ or } 0 & \text{ if } 3 \equiv 5p \mod 8 & \quad \text{ i.e. } p \equiv 7 \mod 8 \\
	p \equiv 5 \bmod 8: & \quad T_p|\Delta^5 = \Delta \text{ or } 0 & \text{ if } 1 \equiv 5p \mod 8 & \quad \text{ i.e. } p \equiv 5 \mod 8 \\
...
$$
So $c(3p) = \#\{m_1, m_2, m_3, m_4, m_5 \text{ odds } | m_1^2 + m_2^2 + m_3^2 + m_4^2 + m_5^2 = 3p\} \mod 2$ corresponds ($\bmod 2$) to the number of ways to write $3p$ as sum of five odd squares.

When looked $\bmod 8$, $m_1^2 + m_2^2 + m_3^2 + m_4^2 + m_5^2 \equiv 5 \mod 8$.
We can check, $p \equiv 7 \mod 8$ so $3p \equiv 5 \mod 8$.
