...
%	\item (1+1+...): No contribution to $c(n)$.
%	\item (1+2+2+2): No contribution to $c(n)$.
%	\item (1+3+3): No contribution to $c(n)$.
%	\item (1+2+4): Contributes to $c(n)$.
%	\item (1+6): Contributes to $c(n)$.
%	\item (2+2+3): No contribution to $c(n)$.
%	\item (2+5): Contributes to $c(n)$.
%	\item (3+4): Contributes to $c(n)$.
%\end{itemize}
%(1+6) have solutions iff (3+4) have
%(1+2+4) and (2+5) to be checked
...
It seems quite random, which makes sense since the Hecke operators depend on primes, and primes appear at random.
...
$$h(k) = n_3(k) + n_5(k).$$
The definition of $n_3$ and $n_5$ is equivalent to the following:
\begin{itemize}
...
It is straightforward, from the definition, that $n_3$ and $n_5$ (therefore also $h$) are the same for an even $2m$ and the next number $2m+1$.
Explicitly:
$$
\left. 
\begin{array}{rc}
...
Moreover, it is clear from the definition that $m_3$, $m_5$ and $h$ preserve addition of powers of 2, that is:
$$
\begin{array}{rcl}
...
\begin{proof}
	...
	The proposition also implies that $h(g) > -\infty$, i.e. $g \neq 0$.
...
From this follows a new remark, which is useful to estimate computation times:
